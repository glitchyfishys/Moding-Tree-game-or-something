...
addLayer("CZ", {
    ...
    effect(){ return Decimal.pow(player[this.layer].points, 5).max(1);},
    effectDescription(){
    ...
    upgrades: {
        ...
        31: {
            canAfford: () =>{
                return player.points.gt(1e120);
            },
            pay: () => {player.points = player.points.sub(1e120)},
...
addLayer("DN", {
    ...
    effect(){
        return Decimal.pow(player[this.layer].points, 2).max(1);
    },
    effectDescription(){
        return `${format(this.effect())} Dynamic Crytal${hasMilestone("DN", 0) ? " and VS" : ""} gain`;
    },
    color: "#dB0013",
    glowColor: "#dB0013",
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
        ...
        gain = gain.mul(prestigeEffect("DMC")).min(10000);
...
addLayer("INFINITY", {
    ...
    effect(){
        return Decimal.pow(player[this.layer].points, 0.3).max(1);
    },
    effectDescription(){
...
addLayer("DMC", {
    ...
    passiveGeneration() {return hasMilestone("VX", 8) ? 10 : 0},
    effect(){
        return (hasMilestone("DMC", 2) ? Decimal.pow(player[this.layer].points, 3.6).max(1) : decimalOne).pow(hasMilestone("DMC", 3) ? 2 : 0);
    },
    effectDescription(){
...
addLayer("VX", {
    ...
    effect(){
        return (hasMilestone("VX", 2) ? Decimal.pow(player[this.layer].points, hasMilestone("EMS", 5) ? (hasMilestone("VX",9) ? 4 : 2) : 0.6).max(1) : decimalOne).mul(hasMilestone("VX",6) ? 2 : 1);
    },
    effectDescription(){
    ...
    infoboxes: {
        help: {
            body: "you will unlock more challenges after reset",
        },
    },
    milestones: {
        0: {
        ...
        2: {
            ...
            effectDescription: "Chaos Cores give a power to VS gain but it is square rotted first",
...
addLayer("CURSE", {
    ...
    infoboxes: {
        help: {
            body: "each Cursed challenge includes all the previous ones",
        },
    },
    challenges: {
        11: {
...
addLayer("EMS", {
    ...
    milestones: {
        0: {
            ...
            effectDescription: "/1.5 OR cost exponent and lower SC6",
